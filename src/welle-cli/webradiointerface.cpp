...
bool WebRadioInterface::dispatch_client(Socket&& client)
{
    ...
    if (not req.valid) {
        ...
        if (req.is_get) {
            if (req.url == "/") {
            ...
            else if (req.url == "/play.m3u") {
                success = send_file(s, "play.m3u", http_contenttype_m3u);
            }
            else if (req.url == "/player.html") {
                success = send_file(s, "player.html", http_contenttype_html);
            }
                ...
                const regex regex_radio(R"(^[/]radio[/]([^ ]+))");
...
bool WebRadioInterface::send_radio(Socket& s, const std::string& stream)
{
    if (stream.size() >= 2)
    {
